...
class CriticalThread(threading.Thread):
    ...
    def wait_exception(cls, timeout: float=None):
        ...
        # checks for exceptions until the timeout is reached or there are no more active threads
        if timeout is not None:
            curr_time = 0
            end_time = time.perf_counter() + timeout
        
            while curr_time <= end_time:
                # exits if thre's no active threads besides main
        ...
        # checks for exceptions untill it finds something or there are no more active threads
        else:
            while True:
                # exits if thre's no active threads besides main
                if threading.active_count() <= 1:
